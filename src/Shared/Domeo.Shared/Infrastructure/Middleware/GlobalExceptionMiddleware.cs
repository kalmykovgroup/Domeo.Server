...
public sealed class GlobalExceptionMiddleware
{
    ...
    private async Task HandleExceptionAsync(HttpContext context, Exception exception)
    {
        ...
        {
            DomeoValidationException validationEx => (
    ...
    private static ApiResponse CreateValidationErrorResponse(DomeoValidationException exception)
    {
        var errors = exception.Errors
            .SelectMany(kvp => kvp.Value.Select(v => string.IsNullOrEmpty(kvp.Key) ? v : $"{kvp.Key}: {v}"))
            .ToList();
